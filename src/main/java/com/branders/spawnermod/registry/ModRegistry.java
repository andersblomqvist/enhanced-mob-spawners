...
public class ModRegistry {
	...
	public static final Item IRON_GOLEM_SPAWN_EGG = new SpawnEggItem(EntityType.IRON_GOLEM, 15198183, 9794134, (new Item.Settings()));
	
	public static void register() {
		
		// Item groups
		ItemGroupEvents.modifyEntriesEvent(ItemGroups.TOOLS).register(entries -> entries.add(SPAWNER_KEY));
		ItemGroupEvents.modifyEntriesEvent(ItemGroups.SPAWN_EGGS).register(entries -> entries.add(IRON_GOLEM_SPAWN_EGG));
		
		Registry.register(Registries.ITEM, new Identifier(SpawnerMod.MOD_ID, "spawner_key"), SPAWNER_KEY);
		Registry.register(Registries.ITEM, new Identifier(SpawnerMod.MOD_ID, "iron_golem_spawn_egg"), IRON_GOLEM_SPAWN_EGG);
